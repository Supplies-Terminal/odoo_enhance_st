...
class StockWarehouseOrderpoint(models.Model):
    ...
    def action_replenish(self):
        ...
        for orderpoint in self:
            ...
            res = super(StockWarehouseOrderpoint, orderpoint).action_replenish()
            ...
            for shortage in shortage_list:
                if shortage['sale_order_id']:
                    po_line = self.env['purchase.order.line'].search([
                        ('order_id', '=', shortage['sale_order_id']),
                ...
                if shortage['production_order_id']:
                    po_line = self.env['purchase.order.line'].search([
                        ('order_id', '=', shortage['production_order_id']),
                        ('product_id', '=', product_id)
                    ], limit=1)
                    if po_line:
