...
class ProductTemplate(models.Model):
    ...
    secondary_uom_rate = fields.Float('Counting Unit to Primary Unit', default=1)
    ...
    def _compute_secondary_uom_desc(self):
        for rec in self:
            if rec.secondary_uom_enabled:
                rec.secondary_uom_desc = "%s (%s%s)" % (rec.secondary_uom.name, rec.secondary_uom_rate, rec.uom_id.name)
            else:
